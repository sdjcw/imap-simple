...
/**
 * Sets the provided flag(s) to the specified message(s).
 *
 * @param {string|Array} uid The messages uid
 * @param {string|Array} flags Either a single string or an array of strings indicating the flags to add to the
 *  message(s).
 ...
 * @memberof ImapSimple
 */
ImapSimple.prototype.setFlags = function (uid, flags, callback) {
    var self = this;

    if (callback) {
        return nodeify(self.setFlags(uid, flags), callback);
    }

    return new Promise(function (resolve, reject) {
        self.imap.setFlags(uid, flags, function (err) {
            if (err) {
                reject(err);
                return;
            }
